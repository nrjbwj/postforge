...
help: ## Display this help message
	@echo "$(BLUE)PostForge - Available Commands$(NC)"
	@echo ""
...
docker-run: ## Run Docker container directly (PORT=3001 to use different port)
	...
	docker run -p $${PORT:-3000}:3000 --env-file .env.local postforge
...
docker-run-detached: ## Run Docker container in detached mode (PORT=3001 to use different port)
	...
	docker run -d -p $${PORT:-3000}:3000 --name postforge --env-file .env.local postforge

docker-stop: ## Stop running Docker container
	@echo "$(YELLOW)Stopping Docker container...$(NC)"
	docker stop postforge 2>/dev/null || true
	docker rm postforge 2>/dev/null || true
...
check-env: ## Check if .env.local file exists
	@if [ ! -f .env.local ]; then \
		...
		echo "NEXT_PUBLIC_POSTS_PER_PAGE=10" >> .env.local; \
		echo "NEXT_PUBLIC_DEFAULT_USER_ID=1" >> .env.local; \
